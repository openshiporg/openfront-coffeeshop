...
interface SectionRendererProps {
  sections: Array<{ id: number; type: string; label: string }>;
      ...
      {sections.map((section, index) => {
        ...
        return (
          <div key={section.type} className="relative">
            {!isLast && <div className="absolute bottom-0 left-3 top-3 w-px bg-border" />}
            ...
            <div className="pl-9">
              <div className="flex flex-wrap gap-2 pb-6">
                {items.map((item) => {
                  ...
                  if (step === 'done') {
                    status = 'completed';
                  } else if (itemErrors[section.type]?.[item]) {
                    status = 'error';
                  ...
                  return <SectionItem key={item} item={item} status={status} errorMessage={errorMessage} />;
                })}
              </div>
            </div>
          </div>
